...
$(document).ready(function () {
  ...
  function validateForm() {
    ...
    if (pincode === "") {
      ...
      displayError(
        "#pincode",
        "Please enter a valid 6-digit pincode with only digits."
      );
      isValid = false;
    }
    ...
    if (phone === "") {
      ...
      displayError(
        "#phone",
        "Please enter a valid 10-digit phone number with only digits."
      );
      isValid = false;
    }
...
$(document).ready(function () {
  $("#placeOrderBtn").on("click", function (event) {
    ...
    console.log(formData, "is getting 2");
    $.ajax({
      type: "POST",
      url: "/user/placeorder",
      data: formData,
      success: function (response) {
        ...
        if (response.placed == true) {
          ...
          window.location.href = `/user/success?id=${id}`;
        } else if (response.wallet == false) {
          swal.fire(
            "Oops, it looks like your wallet balance is too low to place this order !!",
            "",
            "error"
          );
        } else {
  ...
  $.ajax({
    ...
    success: (response) => {
      if (response.placed == true) {
        ...
        window.location.href = `/user/success?id=${orderId}`;
      } else {
        swal.fire({
          positon: "center",
          icon: "error",
